...
假設我們有 A、B 兩個集合，A 集合裡有 n 個元素，B 集合裡有 m 個元素。從兩個集合取出 r 個元素有 $C^{n+m}_r$ 種情況。當我們從集合 A 取出 r 個元素，從集合 B 取出 0 個元素的情況，即 $C^m_0C^n_r$。當我們從集合 A 取出 r-1 個元素，從集合 B 取出 1 個元素的情況，即 $C^m_1C^n_{r-1}$。一直到當我們從集合 A 取出 0 個元素，從集合 B 取出 r 個元素的情況，即 $C^m_rC^n_0$。全部加總即為我們從 A、B 集合取出 r 個元素的情況，即 $C^{n+m}_r=\sum^r_{i=0}C^m_iC^n_{r-i}$。
